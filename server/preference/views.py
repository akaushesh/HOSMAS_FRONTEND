...
class Retain(APIView):
    ...
    def post(self, request):
        ...
        if (lead_curr is None):
            return Response({'error': group.leader.name + ' can not retain because they do not have a prevoius hostel'}, status=status.HTTP_400_BAD_REQUEST)

        members = group.members.all()
        for member in members:
            member_curr = member.current_room
            if (member_curr is None):
                return Response({'error': member.name + ' can not retain because they do not have a prevoius hostel'}, status=status.HTTP_400_BAD_REQUEST)
